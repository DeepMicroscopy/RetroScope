...
Table~\ref{tab:workflow} reports workflow reliability as an integration check rather than as an image-quality metric. A successful autofocus trial required the automation routine to complete and return a final Z position. A successful focus-stack trial required a saved OME-TIFF stack with the expected source planes and metadata. A successful tile-scan trial required a saved scan output with the expected tile planes. These criteria test whether calibrated motion, camera acquisition, file writing, and objective-specific parameters work together repeatedly across objective profiles.

All 120 workflow trials completed successfully: autofocus, focus stacking, and tile scanning each completed 10/10 repetitions for 4$\times$, 20$\times$, 40$\times$, and 63$\times$ objective profiles. Autofocus duration was recorded because it directly depends on the configured Z search range and objective profile. Focus-stacking and tile-scanning durations were not recorded in this evaluation. The 4$\times$ autofocus duration was longer because the low-magnification profile used a larger search range.
...
\hline
4$\times$  & 10/10 & 10/10 & 10/10 & $59.84\pm0.49$ \\
20$\times$ & 10/10 & 10/10 & 10/10 & $17.59\pm0.31$ \\
40$\times$ & 10/10 & 10/10 & 10/10 & $14.66\pm0.17$ \\
63$\times$ & 10/10 & 10/10 & 10/10 & $14.21\pm0.07$ \\
\hline
\end{tabular}
\end{table}
